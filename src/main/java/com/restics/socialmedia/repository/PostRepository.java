...
public class PostRepository {
    ...
    private static final String BASE_POST_QUERY =
            "SELECT p.post_id, p.parent_post_id, u.name, u.user_id, " +
                    "p.numoflikes, p.numOfReplies, p.numofshares, p.created_at, " +
                    "pc.text_content, pc.media_url " +
                    "FROM Post p JOIN Users u ON u.user_id = p.user_id " +
                    "JOIN PostContent pc ON pc.post_id = p.post_id ";
    public List<Post> findAll() {
        return jdbc.query(BASE_POST_QUERY, POST_MAPPER);
    }

    public List<Post> getPostsByAuthorName(String name){
        return jdbc.query(
                BASE_POST_QUERY + "WHERE u.name = ? ORDER BY p.created_at DESC",
                POST_MAPPER,
                name);
    }
    public List<Post> getPostsByAuthorId(int authorID){
        return jdbc.query(
                BASE_POST_QUERY+ " WHERE u.user_id = ? ORDER BY p.created_at DESC",
                POST_MAPPER,
                authorID);
    }
    public Optional<Post> getPostByID(int postID){
        return Optional.ofNullable(jdbc.queryForObject(
                BASE_POST_QUERY + " WHERE p.post_id = ?",
                POST_MAPPER,
                postID));
    }
    ...
    public List<Post> getPostReplies(int postId){
        return jdbc.query(
                BASE_POST_QUERY+ " WHERE p.parent_post_id = ? ORDER BY p.created_at DESC",
                POST_MAPPER,
                postId);
    }
    ...
    public List<Post> getFollowingPosts(int userId){
        return jdbc.query(
                ...
                "p.numoflikes, p.numOfReplies, p.numofshares, p.created_at, " +
                        ...
                        "JOIN PostContent pc ON pc.post_id = p.post_id " +
                        "JOIN Follows f ON u.user_id = f.followed_key "+
    ...
    public int updatePostContent(int postId, String contentType, String content, String mediaUrl) {
        return jdbc.update(
                ...
                contentType, content, mediaUrl, postId
        );
    }
    ...
    // TODO: addreply, addlike, addshare
